namespace MoleMole
{
    ...
    public class MonoDAMeiWS : MonoDAMei
    {
        ...
        protected override void OnCollisionEnter(Collision collision)
        {
            ...
            if ((InLevelData.AVATAR_LAYER == collision.gameObject.layer) && this._isInRun)
            {
        ...
        private void SkillIDChangedCallback(string from, string to)
        {
            ...
            {
                if (CollisionDetectPattern.CircleCollisionDetectBySphere(base.RootNodePosition, 0f, base.FaceDirection, base.config.CommonArguments.CollisionRadius * 3f, ((int) 1) << InLevelData.AVATAR_LAYER).Count > 0)
                {
                    this.SetTrigger(this._attackRunTriggerID);
                    this._isInRun = false;
                }
